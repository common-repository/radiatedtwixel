...
class RadiatedTwixel extends WP_Widget
{
  ...
  {
    $instance = wp_parse_args( (array) $instance, array( 'title' => 'RadiatedTwixel', 'user' => 'radiatedpixel', 'checktime'=>60 ) );
...
<?php
  ...
  function getTwitterFeed($instance){
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "http://api.twitter.com/1/statuses/user_timeline.json?include_entities=false&include_rts=false&exclude_replies=true&screen_name={$instance['user']}&count=5");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    $result = curl_exec($ch);
    curl_close($ch);
    //$result = wp_remote_get("http://api.twitter.com/1/statuses/user_timeline.json?include_entities=false&include_rts=false&exclude_replies=true&screen_name={$instance['user']}&count=5");
    return $result;
  }
}
